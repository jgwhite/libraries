...
public struct LibrariesAPIClient {
    ...
    public func projectDependencies(platform: String, _ name: String, _ version: String = "latest", then: NSDictionary? -> Void) {
        fetch("\(platform)/\(name)/\(version)/dependencies") { then($0 as? NSDictionary) }
    }

    public func projectDependents(platform: String, _ name: String, then: NSArray -> Void) {
        fetch("\(platform)/\(name)/dependents") { then($0 as! NSArray) }
    }

    public func projectDependentRepositories(platform: String, _ name: String, then: NSArray -> Void) {
    ...
    public func githubDependencies(owner: String, _ name: String, then: NSDictionary? -> Void) {
        fetch("github/\(owner)/\(name)/dependencies") { then($0 as? NSDictionary) }
    }

    public func githubProjects(owner: String, _ name: String, then: NSArray -> Void) {
    ...
    func fetch(resource: String, _ params: [String: String] = [String: String](), then: AnyObject? -> Void) {
        ...
        let query = (["api_key=\(apiKey)"] + params.map { "\($0)=\($1)" }).joinWithSeparator("&")
        let url = NSURL(string: "https://libraries.io/api/\(resource)?\(query)")!
